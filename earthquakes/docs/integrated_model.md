...
Интегрированная модель (`main_integrated.py`) — это улучшенная версия базовой модели Пуассона для прогнозирования землетрясений. Она объединяет три компонента:

1. **Bayesian Poisson (Gamma-Poisson)** — учёт неопределённости в параметре λ
2. **ETAS-коррекция** — учёт кластеризации афтершоков
3. **Исторический fit** — параметры из реальных данных 1900-2024
...
3. Корректируем параметры:
   effective_β = β_post / (1 + etas_λ/λ_mean)

4. Вычисляем вероятность через Negative Binomial:
...
```bash
# Только Bayesian (без ETAS)
python main_integrated.py --no-etas

# Простой Пуассон с ETAS
python main_integrated.py --no-bayesian
...
2. **Региональная независимость** — модель не учитывает региональные особенности
3. **Временная стационарность** — предполагается постоянная базовая частота
4. **M9.0+/M10.0+** — Bayesian отключён из-за недостатка данных
...
- `main_integrated.py` — интегрированная модель (Bayesian + ETAS)
...
### v1.2.0 (2026-01-05)
- **УДАЛЁН:** Лунный модификатор
  - Бэктест на 43 годах данных (1980-2023) показал нулевое влияние на качество
  - Brier Score с лунным и без лунного модификатора идентичен (0.1128)
  - Компонент добавлял сложность без пользы для предсказаний
  - Удалена зависимость от `ephem`
...
- ~~Лунный модификатор (ephem)~~ — удалён в v1.2.0
