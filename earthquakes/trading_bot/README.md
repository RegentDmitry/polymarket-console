...
| `--data-dir` | | Директория для данных | `trading_bot/data` |

## Примеры

```bash
# Сканирование каждую минуту
python -m trading_bot --interval 1m

# Строгие параметры входа
python -m trading_bot --min-edge 0.02 --min-apy 0.50

# Полный пример
python -m trading_bot --live --interval 5m --min-edge 0.05 --min-apy 0.25 --max-positions 10
```
...
Запускайте из директории `earthquakes`, не из `trading_bot/`.
