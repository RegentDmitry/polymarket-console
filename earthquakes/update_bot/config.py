...
class UpdateBotConfig:
    ...
    # Paths
    markets_json: Path = Path("earthquake_markets.json")
...
def parse_args() -> UpdateBotConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Polymarket Earthquake Markets Update Bot",
...
Examples:
  ...
  python -m update_bot --interval 12h    # Update every 12 hours
    ...
    parser.add_argument(
        "--markets-json",
        type=Path,
        default=Path("earthquake_markets.json"),
        help="Path to earthquake markets JSON file. Default: earthquake_markets.json"
    )
