...
- Добавлена панель мониторинга источников с таймерами (monitor_bot/ui/app.py)
- EMSC collector теперь заполняет reported_at для корректного Edge Time

Новая функциональность:
- Главное окно разделено на 2 части (события слева, источники справа)
- Панель источников показывает:
  - Статус каждого источника (Idle / Syncing)
  - Таймер обратного отсчета до следующей синхронизации
  - Индикаторы синхронизации в real-time
  - Текущее UTC время (обновляется каждую секунду)
- Отслеживание "висячих" событий (которые USGS может не подтвердить):
  - Pending - ожидание USGS (< 6 часов)
  - Delayed 8h - задержка USGS (6-24 часа)
  - Unlikely - USGS вряд ли подтвердит (> 24 часов)

Осталось протестировать:
- Новый UI с панелью источников работает корректно
- Таймеры обновляются каждую секунду
- Индикаторы синхронизации показываются правильно
