"""
Crypto bot configuration and CLI argument parsing.
"""
...
@dataclass
class BotConfig:
    """Main bot configuration for crypto trading."""
    ...
    markets_json: Path = field(default_factory=lambda: Path("crypto_markets.json"))
...
def parse_args() -> BotConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Polymarket Crypto Trading Bot (BTC/ETH)",
...
Examples:
  ...
  python -m trading_bot --interval 5m            # scan every 5 minutes
    ...
    parser.add_argument(
        ...
        type=int,
        default=150000,
        help="Number of Monte Carlo paths. Default: 150000"
    )

    parser.add_argument(
        "--mc-pricing",
        action="store_true",
        help="Use MC simulation (~30s) instead of fast analytical (default)"
    )
    ...
    parser.add_argument(
        ...
        help="Path to crypto markets JSON. Default: crypto_markets.json"
    )
